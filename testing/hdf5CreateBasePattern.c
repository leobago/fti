...
int main() {
    // create hdf5 file
    hid_t file_id = H5Fcreate("pattern.h5", H5F_ACC_TRUNC, H5P_DEFAULT,
     H5P_DEFAULT);
    ...
    // UIntegers datatype
    hid_t uIntsGroup = H5Gcreate2(allIntsGroup, "Unsigned Integers",
     H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
    ...
    H5Tcommit(file_id, "Chars", charsType, H5P_DEFAULT, H5P_DEFAULT,
     H5P_DEFAULT);

    hsize_t dimChars = 2;

    H5LTmake_dataset(charsAndFloatsGroup, "chars", 1, &dimChars,
     charsType, charVars);
    ...
    H5Tinsert(intType, "int16_t", offsetof(Integers, shortInteger),
     H5T_NATIVE_SHORT);
    H5Tinsert(intType, "int", offsetof(Integers, integer), H5T_NATIVE_INT);
    H5Tinsert(intType, "int32_t", offsetof(Integers, longInteger),
     H5T_NATIVE_LONG);

    H5Tcommit(intsGroup, "struct Integers", intType, H5P_DEFAULT,
     H5P_DEFAULT, H5P_DEFAULT);

    hsize_t dimIntegers = 2;
    hsize_t dimIntegers2 = 1;

    H5LTmake_dataset(allIntsGroup2, "ints", 1, &dimIntegers,
     intType, intVars);
    H5LTmake_dataset(allIntsGroup2, "ints2", 1, &dimIntegers2,
     intType, &intVars2);
    ...
    H5Tinsert(uIntType, "unsigned short int",
     offsetof(UIntegers, shortInteger), H5T_NATIVE_USHORT);
    H5Tinsert(uIntType, "unsigned int",
     offsetof(UIntegers, integer), H5T_NATIVE_UINT);
    H5Tinsert(uIntType, "unsigned long int",
     offsetof(UIntegers, longInteger), H5T_NATIVE_ULONG);

    H5Tcommit(uIntsGroup, "struct UIntegers", uIntType, H5P_DEFAULT,
     H5P_DEFAULT, H5P_DEFAULT);

    hsize_t dimUIntegers = 1;

    H5LTmake_dataset(allIntsGroup2, "unsigned ints", 1 , &dimUIntegers,
     uIntType, &uintVars);
    ...
    H5Tinsert(floatsType, "double", offsetof(Floats, doublePrec),
     H5T_NATIVE_DOUBLE);
    // H5Tinsert(floatsType, "long double", offsetof(Floats,
    // longDoublePrec), H5T_NATIVE_LDOUBLE);

    H5Tcommit(charsAndFloatsGroup, "struct Floats", floatsType,
     H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);

    hsize_t dimFloats = 1;

    H5LTmake_dataset(charsAndFloatsGroup, "floats", 1 , &dimFloats,
     floatsType, &floatVars);
    ...
    H5Tinsert(allIntsType, "struct Integers array",
     offsetof(AllInts, integers), IntegersArray);
    H5Tinsert(allIntsType, "struct UIntegers array",
     offsetof(AllInts, uIntegers), UIntegersArray);

    H5Tcommit(allIntsGroup, "struct AllInts", allIntsType,
     H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);

    hsize_t dimAllInts = 1;

    H5LTmake_dataset(allIntsGroup, "all ints", 1 , &dimAllInts,
     allIntsType, &allIntVars);
    ...
    H5Tinsert(floatsCharsType, "struct Chars array",
     offsetof(FloatsChars, chars), CharsArray);
    ...
    H5LTmake_dataset(charsAndFloatsGroup, "floats and chars", 1 ,
     &dimFloatsChars, floatsCharsType, &floatCharVars);
    ...
    H5Tinsert(allTypesType, "struct AllInts",
     offsetof(AllTypes, allInts), allIntsType);
    H5Tinsert(allTypesType, "struct FloatsChars",
     offsetof(AllTypes, floatsChars), floatsCharsType);
    ...
    H5LTmake_dataset(root_group_id, "all types2D", 2 , dimAllTypes,
     allTypesType, allTypesVar);
