...
int verifyChars(Chars* in, int shift, int rank, char* name) {
    ...
    for (i = 0; i < 2; i++) {
        for (j = 0; j < 3; j++) {
            for (k = 0; k < 4; k++) {
                for (l = 0; l < 5; l++) {
                    if (in->uChars[i][j][k][l] != 'a' +
                     ((i + j + k + l) + shift) % 25) {
                        printf("[ %06d ] : %s.uChars[%d][%d][%d][%d] ="
                            " %u should be %u \n", rank, name,
                                i, j, k, l, in->uChars[i][j][k][l],
                                 'a' + ((i + j + k + l) + shift) % 25);
                        return VERIFY_FAILED;
                    }
                }
...
int main(int argc, char** argv) {
    ...
    // Create groups for types
        ...
        // UIntegers datatype
        hid_t uIntsGroup = H5Gopen(allIntsGroup, "Unsigned Integers",
         H5P_DEFAULT);

        // Chars And Floats datatype
        hid_t charsAndFloatsGroup = H5Gopen(file_id, "Chars and Floats",
         H5P_DEFAULT);

        // AllIntegers datatype
        hid_t allIntsGroup2 = H5Gopen(file_id, "All Integers for Dataset",
         H5P_DEFAULT);

    // open types
    hid_t chars_id = H5Topen(file_id, "Chars", H5P_DEFAULT);
    hid_t floats_id = H5Topen(charsAndFloatsGroup, "struct Floats",
     H5P_DEFAULT);
    hid_t floatChars_id = H5Topen(charsAndFloatsGroup, "struct FloatsChars",
     H5P_DEFAULT);
    ...
    // read datasets
    Chars charVars[2];
    herr_t res = H5LTread_dataset(charsAndFloatsGroup, "chars",
     chars_id, &charVars);
    if (res != 0) printf("Cannot read dataset!\n");

    Floats floatVars;
    res = H5LTread_dataset(charsAndFloatsGroup, "floats",
     floats_id , &floatVars);
    if (res != 0) printf("Cannot read dataset!\n");

    FloatsChars floatCharVars;
    res = H5LTread_dataset(charsAndFloatsGroup, "floats and chars",
     floatChars_id, &floatCharVars);
    if (res != 0) printf("Cannot read dataset!\n");
    ...
    UIntegers uintVars;
    res = H5LTread_dataset(allIntsGroup2, "unsigned ints",
     uintegers_id, &uintVars);
    if (res != 0) printf("Cannot read dataset!\n");

    AllInts allIntVars;
    res = H5LTread_dataset(allIntsGroup, "all ints",
     allInts_id, &allIntVars);
    if (res != 0) printf("Cannot read dataset!\n");

    AllTypes allTypesVar[2][2];
    res = H5LTread_dataset(file_id, "all types2D",
     allTypes_id, &allTypesVar);
    if (res != 0) printf("Cannot read dataset!\n");

    Integers intVars2;
    res = H5LTread_dataset(allIntsGroup2, "ints2",
     integers_id, &intVars2);
    if (res != 0) printf("Cannot read dataset!\n");
    ...
    res1 += verifyAllTypes((allTypesVar[0] + 1), 4, world_rank,
     "allTypesVar[0][1]");
    res1 += verifyAllTypes(allTypesVar[1], 3, world_rank, "allTypesVar[1][0]");
    res1 += verifyAllTypes((allTypesVar[1] + 1), 4, world_rank,
     "allTypesVar[1][1]");
