...
int checkFileSizes(int* mpi_ranks, int world_size, int global_world_size,
 int level, int fail) {
    ...
    for (j = 0; j < nodes; j++) {
        if (level == 4) {
            snprintf(path, sizeof(path), "./Global/%s/l4", exec_id);
        } else {
            snprintf(path, sizeof(path), "./Local/node%d/%s/l%d", j,
             exec_id, level);
        }
            ...
            while ((ent = readdir(dir)) != NULL) {
                if (strstr(ent->d_name , "Rank") != NULL) {
                    ...
                    if (fail) {
                        ...
                        lastCheckpointIter = (ITERATIONS - 1) -
                         (ITERATIONS - 1) % ITER_CHECK;
                    }

                    expectedSize += ((rank + 1) * INIT_SIZE +
                     lastCheckpointIter * rank  * 1024) * sizeof(int32_t);
...
int main(int argc, char** argv) {
    ...
    // verify args
    if (init(argv, &checkpoint_level, &fail, &check_sizes)) return 0;
    ...
    MPI_Gather(&global_world_rank, 1, MPI_INT, mpi_ranks, 1,
     MPI_INT, 0, FTI_COMM_WORLD);
