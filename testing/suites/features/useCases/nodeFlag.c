...
int verify(int world_size, int nbNodes) {
  FILE* fp;
  // Searching this string in log file
  ...
  for (i = 0; i < world_size; i++) {
    snprintf(strtmp, STR_BUF_SIZE, "./log%d.txt", i);
    ...
    while (fgets(temp, 256, fp) != NULL) {
      if ((strstr(temp, str)) != NULL) {
        ...
        sscanf(temp,
               "[FTI Debug - %06d] : Has nodeFlag = 1 and nodeID = %d. "
               "CkptLvel = %d.",
               &processIDtmp, &nodeIDtmp, &ckptLveltmp);
        ...
        if (ckptLvel[ckptLveltmp][nodeIDtmp] != processIDtmp) {
          fprintf(stderr, "Lv%d: Node %d; Process %d\n", ckptLveltmp, nodeIDtmp,
                  processIDtmp);
          fprintf(stderr,
                  "The node %d on checkpoint level %d has more than 1 nodeFlag "
                  "process: %d and %d.\n",
  ...
  for (i = 1; i < 5; i++) {
    for (k = 0; k < nbNodes; k++) {
      if (ckptLvel[i][k] == -1) {
        fprintf(stderr,
                "Node %d on checkpoint level %d don't have signed process!\n",
                k, i);
        return 1;
      }
    }
...
int main(int argc, char** argv) {
  ...
  // Creating log files
  char str[256];
  snprintf(str, STR_BUF_SIZE, "./log%d.txt", global_world_rank);
