...
int verifyChars(Chars* in, int shift, int rank, char* name) {
    ...
    for (i = 0; i < 2; i++) {
        for (j = 0; j < 3; j++) {
            for (k = 0; k < 4; k++) {
                for (l = 0; l < 5; l++) {
                    if (in->uChars[i][j][k][l] != 'a' + ((i + j + k + l) +
                     shift) % 25) {
                        printf("[ %06d ] : %s.uChars[%d][%d][%d][%d] = %u "
                            "should be %u \n", rank, name, i, j, k, l,
                            in->uChars[i][j][k][l], 'a' +
                            ((i + j + k + l) + shift) % 25);
                        return VERIFY_FAILED;
        ...
        for (j = 0; j < 1024; j++) {
            if (in->bytes[i].longs[j] != (j + 1) * 2 + shift) {
                printf("[ %06d ] : %s.bytes[%d].longs[%d] = %ld"
                " should be %d \n", rank, name, i, j, in->bytes[i].longs[j],
                (j + 1) * 2 + shift);
                return VERIFY_FAILED;
            }
        }
...
int verifyFloats(Floats * in, int shift, int rank, char* name) {
    ...
    //     name, in->longDoublePrec, 1234.125 + shift);
    //    return VERIFY_FAILED;
    // }
    return VERIFY_SUCCESS;
}
...
int main(int argc, char** argv) {
    ...
    FTI_AddSimpleField(&IntegersDef, &FTI_SHRT,
     offsetof(Integers, shortInteger), 0, "int16_t");
    FTI_AddSimpleField(&IntegersDef, &FTI_INTG,
     offsetof(Integers, integer), 1, "int");
    FTI_AddSimpleField(&IntegersDef, &FTI_LONG,
     offsetof(Integers, longInteger), 2, "int32_t");

    FTI_InitComplexType(&IntegersType, &IntegersDef,
     3, sizeof(Integers), "struct Integers", &intsGroup);

    // Unsigned integers
    FTIT_complexType UIntegersDef;
    FTIT_type UIntegersType;
    FTI_AddSimpleField(&UIntegersDef, &FTI_USHT,
     offsetof(UIntegers, shortInteger), 0, "uint16_t");
    FTI_AddSimpleField(&UIntegersDef, &FTI_UINT,
     offsetof(UIntegers, integer), 1, "unsigned int");
    FTI_AddSimpleField(&UIntegersDef, &FTI_ULNG,
     offsetof(UIntegers, longInteger), 2, "uint32_t");

    FTI_InitComplexType(&UIntegersType, &UIntegersDef,
     3, sizeof(UIntegers), "struct UIntegers", &uIntsGroup);

    // Floats
    FTIT_complexType FloatsDef;
    FTIT_type FloatsType;
    FTI_AddSimpleField(&FloatsDef, &FTI_SFLT,
     offsetof(Floats, singlePrec), 0, "float");
    FTI_AddSimpleField(&FloatsDef, &FTI_DBLE,
     offsetof(Floats, doublePrec), 1, "double");
    // FTI_AddSimpleField(&FloatsDef, &FTI_LDBE,
    // offsetof(Floats, longDoublePrec), 2, "long double");

    FTI_InitComplexType(&FloatsType, &FloatsDef, 2,
     sizeof(Floats), "struct Floats", &charsAndFloatsGroup);
    ...
    dimLength[0] = 5;
    FTI_AddComplexField(&AllIntsDef, &IntegersType,
     offsetof(AllInts, integers), 1, dimLength, 0, "struct Integers array");

    dimLength[0] = 4;
    FTI_AddComplexField(&AllIntsDef, &UIntegersType,
     offsetof(AllInts, uIntegers), 1, dimLength, 1, "struct UIntegers array");
    ...
    FTI_AddComplexField(&FloatsCharsDef, &FloatsType,
     offsetof(FloatsChars, floats), 1, dimLength, 0, "struct Floats array");

    dimLength[0] = 2;
    FTI_AddComplexField(&FloatsCharsDef, &CharsType,
     offsetof(FloatsChars, chars), 1, dimLength, 1, "struct Chars array");

    FTI_InitComplexType(&FloatsCharsType, &FloatsCharsDef, 2,
     sizeof(FloatsChars), "struct FloatsChars", &charsAndFloatsGroup);

    // All types aggregated
    FTIT_complexType AllTypesDef;
    FTIT_type AllTypesType;

    FTI_AddSimpleField(&AllTypesDef, &AllIntsType, offsetof(AllTypes, allInts),
     0, "struct AllInts");
    ...
    if (fail == 1) {
        ...
        int general_tag = (int)iniparser_getint(ini, "Advanced:general_tag",
         2612);
        int res;
        ...
        // There is no FTI_Finalize(), because want to
        // recover also from L1, L2, L3
        MPI_Finalize();
    } else {
        FTI_Recover();
        ...
        res += verifyFloatsChars(&floatCharVars, 2, world_rank,
         "floatCharVars");
        res += verifyAllTypes(allTypesVar[0], 3, world_rank,
         "allTypesVar[0][0]");
        res += verifyAllTypes((allTypesVar[0] + 1), 4, world_rank,
         "allTypesVar[0][1]");
        res += verifyAllTypes(allTypesVar[1], 3, world_rank,
         "allTypesVar[1][0]");
