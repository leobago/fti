...
void init_srand() { srand(get_seed()); }
...
void init(char *fti_cfgfile, dcp_info_t *info, unsigned long alloc_size) {
  ...
  info->oldsize =
      (unsigned long *)malloc(info->nbuffer * sizeof(unsigned long));
  info->hash =
      (unsigned char **)malloc(info->nbuffer * sizeof(unsigned char *));
  int idx;
...
void xor_data(int id, dcp_info_t *info) {
  ...
  for (idx = 0; idx < info->nbuffer; ++idx) {
    ...
    info->xor_info[id].nunits[idx] =
        ((unsigned long)(info->xor_info[id].share * eff_size)) / UI_UNIT;
    assert(info->xor_info[id].nunits[idx] * UI_UNIT < info->size[idx]);
    unsigned long idxul;
    // DBG_MSG_APP("size[%d]: %lu, offset: %d, udsize:
    // %lu",0,idx,info->size[idx],info->xor_info[id].offset[idx],
  ...
  ckptsize +=
      sizeof(int) + NUM_DCKPT * sizeof(xor_info_t) + sizeof(unsigned int);
  long dcpStats[2];
  ...
  DBG_MSG_APP("changed: %lu, of: %lu, expected dCP update (min): %.2lf", 0,
              dcpStats[1], dcpStats[0],
              100 * ((double)dcpStats[1]) / dcpStats[0]);
}
...
void invert_data(dcp_info_t *info) {
  ...
  for (id = 0; id < NUM_DCKPT; ++id) {
    ...
    for (idx = 0; idx < info->nbuffer; ++idx) {
      ...
      // info->xor_info[id].offset[idx]; unsigned long nunits = ((unsigned
      // long)(info->xor_info[id].share * eff_size))/UI_UNIT;
      unsigned long idxul;
      char *ptr = (char *)(void *)((uintptr_t)info->buffer[idx] +
                                   (uintptr_t)info->xor_info[id].offset[idx]);
      unsigned long cnt = 0;
...
void allocate_buffers(dcp_info_t *info, unsigned long alloc_size) {
  ...
  for (idx = 0; idx < info->nbuffer; ++idx) {
    ...
    // DBG_MSG_APP("idx: %d, allocated (variable): %lu, share: %.2lf%%",0,idx,
    // info->size[idx], share*100);
  }
  if (allocated != alloc_size) {
    ...
    if (info->buffer[idx - 1] == NULL) {
      EXIT_STD_ERR("idx: %d, cannot reallocate %lu bytes", idx - 1,
                   info->size[idx - 1]);
    }
  }
  unsigned long ckptsize = allocated + sizeof(int) +
                           NUM_DCKPT * sizeof(xor_info_t) +
                           sizeof(unsigned int);
  // DBG_MSG_APP("allocated (total): %lu, [ckptsize: %lu]", -1, allocated,
  // ckptsize);
  assert((alloc_size == allocated));
}
unsigned long reallocate_buffers(dcp_info_t *info, unsigned long _alloc_size,
                                 enum ALLOC_FLAGS ALLOC_FLAG) {
  unsigned long alloc_size;
  if (ALLOC_FLAG == ALLOC_RANDOM) {
    // srand(get_seed());
    alloc_size =
        ((unsigned long)(((uint64_t)rand() << 32) | rand())) % _alloc_size + 1;
  } else {
  ...
  for (idx = 0; idx < info->nbuffer; ++idx) {
    ...
    info->size[idx] = (unsigned long)(share * alloc_size);
    allocated += info->size[idx];
    // DBG_MSG_APP("idx: %d, re-allocated (variable) : %lu, share:
  ...
  unsigned long ckptsize = allocated + sizeof(int) +
                           NUM_DCKPT * sizeof(xor_info_t) +
                           sizeof(unsigned int);
  // DBG_MSG_APP("re-allocated (total): %lu, [ckptsize: %lu]", -1, allocated,
  // ckptsize);
  assert((alloc_size == allocated));
  return alloc_size;
}
...
//        (uintptr_t)info->size[idx]; while ( ptr < ptr_e ) {
