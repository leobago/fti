...
int main(int argc, char **argv) {
  ...
  FTIT_hsize_t offset[] = {
      ((rank / ((int)sqrt(size))) % ((int)sqrt(size))) * ldim0,
      (rank % ((int)sqrt(size))) * ldim1};
  ...
  struct STRUCT **sdata =
      (struct STRUCT **)malloc(sizeof(struct STRUCT *) * ldim0);
  for (i = 0; i < ldim0; ++i) {
  ...
  for (i = 0; i < ldim0; ++i) {
    ...
    sdata[i] =
        (struct STRUCT *)realloc(sdata[i], sizeof(struct STRUCT) * ldim1);
    FTI_Protect(i, data[i], ldim1, FTI_INTG);
    FTI_Protect(i + ldim0, sdata[i], ldim1, FTI_NEW_STRUCT);
