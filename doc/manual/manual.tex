...
int main(int argc, char** argv){
    MPI_Init(&argc, &argv);
    //assuming 1st argument is path to config.fti
...
\end{center}
\newpage
\section{\asciifamily FTI\_InitType( $\dots$ )}\label{sec:ftisnapshot}
\begin{framed}
\begin{itemize}
...
\end{description}
\subsection*{Description}
This function initalizes a data type. A variable's type which isn't defined by default by FTI should be added using this function before adding this variable to protected variables.
\subsection*{Example}
\begin{center}
\begin{lstlisting}[frame=single]
typedef struct A {
...
\begin{framed}
\begin{itemize}
\item[--] Writes values of protected runtime variables to a checkpoint file of requested level. Depending on the checkpoint level file is stored in local, partner node or global directory. Checkpoint's ID must be positive integer and it is assumed that more recent checkpoint has greater ID. 
\end{itemize}
\end{framed}
\subsection*{Definition}
\begin{lstlisting}[frame=single]
...
\end{description}
\subsection*{Description}
This function is used to store current values of protected variables into a checkpoint file.
\subsection*{Example}
\begin{center}
\begin{lstlisting}[frame=single]
int i;
