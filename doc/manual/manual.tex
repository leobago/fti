...
\end{center}
\subsection*{Description}
This configuration is made of default values (see: \ref{ch:configuration}). FTI processes are not created (Head = 0, notice: if there is no FTI processes, all checkpoints must be done synchronously, thus Inline\_L2, Inline\_L3 and Inline\_L4 are set to 1), last checkpoint won't be kept (keep\_last\_ckpt = 0), FTI\_Snapshot() will take L1 checkpoint every 3 min,L2 - every 5 min, L3 - every 7 min and L4 - every 11 min, FTI will print errors and some few important information (Verbosity = 2) and IO mode is set to POSIX (ckpt\_io = 1). This is a normal launch of a job, because failure is set to 0 and Exec\_ID is NULL. Local\_test = 1 makes this a local test.
\newpage
\subsection{Using FTI processes (synchronously)}\label{subsec:configftiproccessynch}
\begin{center}
\begin{lstlisting}[frame=single]
[Basic]
...
\end{center}
\subsection*{Description}
FTI processes are created (Head = 1) checkpointing is done synchronously, thus Inline\_L2, Inline\_L3 and Inline\_L4 are set to 1.
\newpage
\subsection{Using FTI processes (asynchronously)}\label{subsec:configftiproccesasynch}
\begin{center}
\begin{lstlisting}[frame=single]
[Basic]
...
\end{center}
\subsection*{Description}
FTI processes are created (Head = 1) checkpointing is done synchronously, thus Inline\_L2, Inline\_L3 and Inline\_L4 are set to 0. Note that it is possible to make one checkpoint level synchronus and other asynchronus (e.g. L2 synch., L3, L4 asynch.).
\newpage
...
\end{center}
\subsection*{Description}
FTI\_Snapshot() makes a checkpoint by given time and also recovers data after a failure, thus makes the code shorter. Checkpoints intervals can be set in configuration file (see: ref{subsec:ckptl1}.
\newpage
