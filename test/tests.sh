...
startTestCorr () {
	...
	mpirun $MPIARGS -n $3 ./$1 config.fti $4 1 $check_sizes &> logFile1
	rtn=$?
	if [ $rtn != 0 ]; then
	...
	printResume $1 $2 $4
	mpirun $MPIARGS -n $3 ./$1 config.fti $4 0 $check_sizes &> logFile2
...
startTest () { #$1 - test name $2 - config name; $3 - number of processes; $4 - checkpoint level; $5 - ckpt io
	...
	mpirun $MPIARGS -n $3 ./$1 config.fti $4 1 &> logFile1
	rtn=$?
	if [ $rtn != 0 ]; then
		cat logFile1
		exit $rtn
	...
	printResume $1 $2 $4
	mpirun $MPIARGS -n $3 ./$1 config.fti $4 0 &> logFile2
	rtn=$?
	if [ $rtn != 0 ]; then
		cat logFile2
		exit $rtn
