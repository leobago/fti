...
void communicate(int numprocs, int rank, long xElem, long yElem, double *in, double *out, double *hallowArrays[], MPI_Comm comm){
...
int main(int argc, char *argv[])
{
  ...
  long sizePerNode = (atol(argv[1]) *1024*512)/sizeof(double);
  M = sqrt(sizePerNode);
  ...
  for ( i = 0; i < 5; i++){
    ...
    communicate(nbProcs, rank, M+2, nbLines+2 , h, g, hallowArrays, FTI_COMM_WORLD);
    localError = executekernel(M+2, nbLines+2 , h, g, hallowArrays,  dError, lError, rank);
