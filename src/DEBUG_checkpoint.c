 ...
 **/
/*-------------------------------------------------------------------------*/
int FTI_UpdateIterTime(FTIT_execution* FTI_Exec)										// ### first call inits FTI_Exec->iterTime = MPI_Wtime() and sets FTI_Exec->ckptIcnt=1
{	
    int nbProcs, res;
    char str[FTI_BUFS];							
    ...
    /** Do not enter before the second call. 
     *  For the first call only ckptIcnt increases by 1 **/
    ...
    if (FTI_Exec->ckptIcnt > 0) { 																							
		...
		/** latIterTime and totalIterTime are set before **/
		
        if (FTI_Exec->ckptIcnt % FTI_Exec->syncIter == 0) { 						// for gt > 1 always fullfilled, since syncIter (=1) never changes													
           
			/** set meanIterTime **/
			
            FTI_Exec->meanIterTime = FTI_Exec->totalIterTime / FTI_Exec->ckptIcnt;											
            
            /** set globMeanIter(time) **/            
            ...
            /** assuming globMeanTime = const --> if or else for all executions. for gt > 1 if and for gt < 1 else.  **/
            
            if (FTI_Exec->globMeanIter > 60) {										// gt > 1min							
				FTI_Exec->ckptIntv = 1;
            }
            else {																	// gt <= 1min  (set to 1 iteration / minute)
                FTI_Exec->ckptIntv = rint(60.0 / FTI_Exec->globMeanIter);
            }
            
            /** consider only buggy case when globMeanIter > 60 **/
            
            res = FTI_Exec->ckptLast + FTI_Exec->ckptIntv;							// res++ (if gt > 1min)  (set to approx ckptIntv [iteration/minute])
            
            /** in first execution of the encapsulating if --> res ++2 **/
            
            if (FTI_Exec->ckptLast == 0) {											// only for the first time (also after recovering)																			
                res = res + 1;
            }
            
            /** in first execution of the encapsulating if --> ckptNext = 2 **/
            ...
            if (FTI_Exec->syncIter < (FTI_Exec->ckptIntv / 2)) {					// for gt > 1 this if is never fullfilled															
