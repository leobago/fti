...
int FTI_Iniparser( FTIT_iniparser* self, const char* inifile, FTIT_inimode mode )
{
    ...
    if ( access(inifile, R_OK) != 0 ) {
        if( mode == FTI_INI_OPEN ) {
            snprintf( err, FTI_BUFS, "Iniparser failed to open meta file ('%s').", inifile );
...
int FTI_IniparserDump( FTIT_iniparser* self )
{
    ...
    if (fd == NULL) {
        ...
        snprintf( err, FTI_BUFS, "Iniparser failed to open file '%s'", self->file);
        FTI_Print( err, FTI_WARN );
    ...
    if (fclose(fd) != 0) {
        ...
        snprintf( err, FTI_BUFS, "Iniparser failed to close file '%s'", self->file);
        FTI_Print( err, FTI_WARN );
